...
public class BmpUtil {
	 ...
	 * @param b
	 * @param matrix
	 * @param bRecycled 是否回收原图b[原b与return不是同一个BMP需要传true]
	 * @return
	 */
	 ...
	 * @return 处理后的Bitmap
	 */
	public static Bitmap getRotateBitmap(Bitmap b, float rotateDegree,boolean bRecycled) {
		if(b==null)
			return null;
		if(rotateDegree==0){
			return b;
	...
	public static Bitmap getScaleBitmap(Bitmap b, int desMax,boolean bRecycled) {
		if(b==null)
			return null;
		int srcMax=b.getHeight()>b.getWidth()? b.getHeight():b.getWidth();
		if (srcMax> desMax) {
	 ...
	 * @return 处理后的Bitmap
	 */
	public static Bitmap getScaleBitmap(Bitmap b, float sx, float sy,boolean bRecycled) {
		if(b==null)
			return null;
		Matrix matrix = new Matrix();
	 ...
	 * @param bitmap
	 *            要处理的Bitmap
	 * @param bRecycled 是否回收原图bitmap[原b与return不是同一个BMP需要传true]
	 * @return 处理后的Bitmap
	 */
